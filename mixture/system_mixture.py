...
class System:
    def __init__(self,num,packing_density,packing_density_0,particle_area,mc,condensed_ratio=0.98,margin=0.2,pre_random=10,
                 device=cpu,shape='A',sdf_file_num=1,mixture=False,concave_mixture_ratio=0.5):
        self.num=num
        ...
        self.concave_mixture_ratio=concave_mixture_ratio
    ...
    def generate_system(self):
        if self.mixture == True:
            ...
            col_spacing=row_spacing=max(col_spacing_A,col_spacing_B,row_spacing_A,row_spacing_B)*1.3
            ...
            positions=[]
            orientation=[]
            typeid=[0]*round(self.num*(1-self.concave_mixture_ratio))+[1]*round(self.num*self.concave_mixture_ratio)
        ...
        print(f"压缩体系完成，{len(positions)} 个有序排列粒子,适宜的移动步长为{self.mc.d['A']},适宜的旋转步长为{self.mc.a['B']}")

    def save_to_gsd(self):
        logger = hoomd.logging.Logger()
        logger.add(self.mc, quantities=['type_shapes'])
        if self.mixture == True:
            gsd_writer = hoomd.write.GSD(filename='./gsd_mixture/P_{:.2f}_{:.2f}_{}.gsd'.format(self.concave_mixture_ratio,self.packing_density_0,self.num),
    ...
    def save_to_gsd_sdf(self):
        ...
        logger.add(self.mc, quantities=['type_shapes'])
        if self.mixture == True:
            gsd_writer = hoomd.write.GSD(filename='./gsd_sdf/mixture/P_{:.2f}_{:.2f}_{}_{}.gsd'.format(self.concave_mixture_ratio,self.packing_density_0,self.num,self.sdf_file_num),
                                        trigger=hoomd.trigger.Periodic(100),
                                        mode='wb',filter=hoomd.filter.All(),
                                        logger=logger)
        elif self.shape=='A':
            gsd_writer = hoomd.write.GSD(filename='./gsd_sdf/convex/P_{:.2f}_{}_{}.gsd'.format(self.packing_density_0,self.num,self.sdf_file_num),
                                        trigger=hoomd.trigger.Periodic(100),
                                        mode='wb',filter=hoomd.filter.All(),
                                        logger=logger)
        elif self.shape=='B':
