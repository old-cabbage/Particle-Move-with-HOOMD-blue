...
def add_data_with_auto_id(new_data, target_class="user", filename="result\sdf\convex\convex_sdf.json"):
    ...
    if data[target_class]:
        max_id = max(item["id"] for item in data[target_class])
    else:
        max_id = 0
...
class System:
    def __init__(self,num,packing_density,packing_density_0,particle_area,mc,condensed_ratio=0.98,margin=0.2,pre_random=10,
                 device=cpu,shape='A',sdf_file_num=1,mixture=False):
        self.num=num
    ...
    def generate_system(self):
        if self.mixture == True:
            ...
            for i in range(rows):
                for j in range(cols):
                    ...
                    orientation.append([np.cos(0.0/2), 0, 0, np.sin(0.0/2)])
                    count += 1
            print(f"成功初始化了 {len(positions)} 个有序排列粒子,密度为{self.packing_density_0}现在开始压缩体系")
        ...
        print(f"粒子数为{self.num}，密度为{self.packing_density_0}的系统压缩完成，{len(positions)} 个有序排列粒子,盒子长{width:.2f},盒子高{height:.2f},行间距={row_spacing:.2f}, 列间距={col_spacing:.2f},适宜的移动步长为{self.mc.d[self.shape]},适宜的旋转步长为{self.mc.a[self.shape]}")
    ...
    def save_to_gsd_sdf(self):
        ...
        logger.add(self.mc, quantities=['type_shapes'])
        if self.shape=='A':
            gsd_writer = hoomd.write.GSD(filename='./gsd_sdf/convex/P_convex_{:.2f}_{}_{}.gsd'.format(self.packing_density_0,self.num,self.sdf_file_num),
                                        trigger=hoomd.trigger.Periodic(100),
                                        mode='wb',filter=hoomd.filter.All(),
                                        logger=logger)
        elif self.shape=='B':
